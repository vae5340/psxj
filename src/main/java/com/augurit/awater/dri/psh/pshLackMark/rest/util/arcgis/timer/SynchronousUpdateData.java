...
public class SynchronousUpdateData {
	...
	@Autowired
	public SynchronousUpdateData(IPshCorrectMarkService correctMarkService){
	...
	public static void updateFeature(){
		synchronized(SynchronousUpdateData.class){
			...
			List<Long> listLack = correctMarkService.getNotSyncForm("psh_lack_mark",null,"3");
			int i=0;
			...
			if(listLack!=null && listLack.size()>0){
				for(Long id : listLack){
					PshLackMarkForm lackForm =  correctMarkService.getLackForm(id);
					if(lackForm!=null){
	...
	public void updateData(){
		...
		for(int k=0;k<listLack.size();k++){
			...
			if(ids!=null){
				PshLackMarkForm lackForm =  correctMarkService.getLackForm(ids);
				FeatureForm feature = DataFormToFeatureConvertor.convertLackVoToForm(lackForm);
				Boolean flags = addFeature(feature);
			}
		}
