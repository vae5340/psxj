...
import com.augurit.awater.dri.psh.pshLackMark.rest.util.arcgis.timer.SynchronousDeleteData;
...
@Service
@Transactional
public class PshCorrectMarkServiceImpl implements IPshCorrectMarkService {
	@Autowired
	...
	public String deleteReport(String reportType, String reportId,String loginName,String phoneBrand) {
		...
		if(StringUtils.isNotBlank(reportType) && StringUtils.isNotBlank(reportId) && StringUtils.isNotBlank(loginName)){
			try {
				OpuOmUserInfo userForm =  omUserService.getOpuOmUserInfoByLoginName(loginName);
				if("add".equals(reportType)){
					PshLackMarkForm lackForm = lackMarkService.get(Long.parseLong(reportId));
					...
					if(!StringUtils.isNotBlank(lackForm.getObjectId())){ // 是否有objectid，有的话就执行删除队列
						...
						//判断接驳井是否关联排水户，已关联的不能删除
						//List<PshDischargerWellForm> wellList= pshDischargerWellService.getFormByObjectId(lackForm.getObjectId());
						List<PshDischargerWellForm> wellList=null;
						if(wellList!=null && wellList.size()>0){
							json.put("code", 400);
							json.put("message", "接驳井已关联排水户，不能删除!");
							return json.toString();
						}
						lackMarkService.delete(lackForm.getId());
						Boolean flags =  httpArcgisClient.deleteFeature(lackForm.getObjectId());
						if(!flags){
						...
						deleteForm.setDeletePerson(userForm.getUserName());
						deleteForm.setDeletePersonId(userForm.getLoginName());
						deleteForm.setPersonUserId(userForm.getUserId().toString());
	...
	public String getParentOrgName(String loginName) {
		...
		if(StringUtils.isNotBlank(loginName)){
			//全部市级单位
			List<OpuOmOrg> listOrg =  psOrgUserService.listOmOrgByRank("14");
			Map<String,String> map =  this.getFrom(loginName);
			for(OpuOmOrg mp :listOrg){
				Map m = new HashMap<>();
				if(mp.getOrgName()!=null){
					if(map.containsKey("parentOrgId")&& map.get("parentOrgId").equals(mp.getOrgId().toString())){
						Map dataMap = new HashMap<>();
						dataMap.put("parentOrgId", map.get("parentOrgId"));
						dataMap.put("parentOrgName", map.get("parentOrgName"));
						json.put("data", dataMap);
					}else{
						m.put("parentOrgName", mp.getOrgName().toString());
						m.put("parentOrgId", mp.getOrgId().toString());
						list.add(m);
					}
				}
			}
	...
	public Map<String, Object> toPartsCount(CorrectMarkForm form,Map<String,Object> map){
		...
		PshLackMarkForm lackForm = new PshLackMarkForm();
	...
	private List<Map<String,Object>> toPartsAndChart(Map<String,Object> map){
		...
		StringBuffer sql = new StringBuffer("select PARENT_ORG_NAME,sum(total) as total from ("+
				"select S1.PARENT_ORG_NAME , count(*) as total from CORRECT_MARK s1 where 1=1");
		...
		sql.append("GROUP BY s1.PARENT_ORG_NAME UNION all select S2.PARENT_ORG_NAME , count(*) as total from Lack_MARK s2 where 1=1");
		if(map.size()>0 && map.containsKey("startTime") && map.containsKey("endTime")){
			String startTime = sim.format(map.get("startTime"));
			String endTime = sim.format(map.get("endTime"));
	 ...
	 * */
	@Override
	public String getCorrectMarkList(OpuOmUserInfo userForm, Page<CorrectMarkForm> page,
	...
	public String getCorrectMarks(String loginName,Page<CorrectMarkForm> page, CorrectMarkForm corrFrom,
		...
		try {
			...
			for (OpuOmOrg omOrgForm :listOrg) {
				if(omOrgForm!=null&&omOrgForm.getOrgRank()!=null){
						...
						if(omOrgForm.getOrgRank().equals("12")){// 直属单位
							...
							while("15".equals(orgGrade)){
								...
								if(orgGrade.equals("1")){
									...
									corrFrom.setParentOrgId(om.getOrgId().toString());
									pg = this.searchFromAndMap(page,corrFrom,map);
								}
							}
						}
	...
	/**
	 * 本接口通用方法，根据loginName获取队伍和直属机构、业务单位
	 * @param loginName
	 * @return map 
	 * */
	public Map<String,String> getFrom(String loginName){
		...
		if(userFrom!=null){
			...
			for(OpuOmOrg from : list){
				if(("11").equals(from.getOrgRank()) && !map.containsKey("teamOrgName")){//巡查组(队伍名称)
					map.put("teamOrgId", from.getOrgId().toString());
					map.put("teamOrgName", from.getOrgName());
				}else if(("12").equals(from.getOrgRank()) && !map.containsKey("directOrgName")){//直属机构
					map.put("directOrgId", from.getOrgId().toString());
					map.put("directOrgName", from.getOrgName());
				}else if(("13").equals(from.getOrgRank()) && !map.containsKey("superviseOrgName")){//监理单位
				...
				if(list.size()<=3 && new Integer(1).equals(from.getOrgLevel())){//市级(也是业主机构)
	 ...
	 */
	@Override
	public String getRepeatReport(OpuOmUserInfo userForm,Page<CorrectMarkForm> page,
			CorrectMarkForm form, Map<String, Object> map) {
		JSONObject json = new JSONObject();
	...
	public Page<CorrectMarkForm> searchFromAndMap(Page<CorrectMarkForm> page, 
		...
		Map<String,Object> values = new HashMap<String,Object>();
		//sql统计数量
		StringBuffer sql = new StringBuffer("from Correct_Mark ps where 1=1");
		// 查询条件
		if(form != null){
...
//				hql.append(" and to_char(ps.markTime,'yyyy-MM-dd hh24:mi:ss') >= :startTime and to_char(ps.markTime,'yyyy-MM-dd hh24:mi:ss') <= :endTime");
	...
	public List<Map<String,Object>> getLatestTen(){
		String sql = "select PARENT_ORG_NAME,DIRECT_ORG_NAME,SUPERVISE_ORG_NAME,"
				+ "TEAM_ORG_NAME,ID,MARK_PERSON,CORRECT_TYPE,MARK_TIME,LAYER_NAME from (select * from AWATER_SWJ.CORRECT_MARK ps where ps.MARK_TIME>sysdate-3 order by ps.MARK_TIME desc) where ROWNUM<=10";
	...
	public Map<String,Object> seeCorrectMark(String path, long id) {
		...
		//correctMarkDao.getSessionFactory().evict(CorrectMark.class,id);
	...
	public void featureToUpdateForm(FeatureForm featureForm){
		if("add".equals(featureForm.getReport_type()) ){
			PshLackMarkForm form = lackMarkService.get(featureForm.getId());
	 ...
	 * */
	@Override
	public String checkForm(String chechState,String checkDesription,OpuOmUserInfo userForm,String isCheck, Long id) {
		JSONObject json = new JSONObject();
...
//				form.setCheckPersonId(userForm.getUser().getUserId().toString());
				form.setCheckPersonId(userForm.getUserMobile());//存手机号
				form.setCheckPerson(userForm.getUserName());
...
//				checkForm.setCheckPersonId(userForm.getUser().getUserId().toString());
				checkForm.setCheckPersonId(userForm.getUserMobile());//存手机号
				checkForm.setCheckPerson(userForm.getUserName());
			...
			PshLackMarkForm form = lackMarkService.get(id);
			if("2".equals(form.getCheckState())){
				json.put("success", false);
				json.put("message", "该数据已审！请刷新重新查看!");
				return json.toString();
...
//				checkForm.setCheckPersonId(userForm.getUser().getUserId().toString());
	...
	public String getNum(OpuOmUserInfo userForm,CorrectMarkForm form,Map<String,Object> map){
		...
		String sql1="select count(*) from( select t.id from correct_mark t where t.check_state='1'"+t_parentOrgId+sqlMoreCorrect.toString()+" union all select k.id from lack_mark k where k.check_state='1'"+k_parentOrgId+sqlMoreLack.toString()+") ";
		String sql2="select count(*) from( select t.id from correct_mark t where t.check_state='2'"+t_parentOrgId+sqlMoreCorrect.toString()+" union all select k.id from lack_mark k where k.check_state='2'"+k_parentOrgId+sqlMoreLack.toString()+") ";
		String sql3="select count(*) from( select t.id from correct_mark t where t.check_state='3'"+t_parentOrgId+sqlMoreCorrect.toString()+"  union all select k.id from lack_mark k where k.check_state='3'"+k_parentOrgId+sqlMoreLack.toString()+") ";
		String sql4="select count(*) from( select t.id from correct_mark t where t.is_add_feature='2'"+t_parentOrgId+sqlMoreCorrect.toString()+"  union all select k.id from lack_mark k where k.is_add_feature='2'"+k_parentOrgId+sqlMoreLack.toString()+") ";
	 ...
	 * */
	@Override
	public String saveAFeatureFormJs(OpuOmUserInfo userForm) {
		JSONObject json = new JSONObject();
		List<CorrectMarkForm> listCorr = new ArrayList<>();
		List<PshLackMarkForm> lackCorr = new ArrayList<>();
		Page pg = new Page();
		pg.setPageSize(100);
		...
		if(pgLack.getResult()!=null){
			...
			for(PshLackMarkForm lackForm : lackCorr){
	 ...
	 * */
	@Override
	public String getCorrectAndLackMarks(OpuOmUserInfo userForm,Page<CorrectMarkForm> page,
			CorrectMarkForm form, Map<String, Object> maps) {
		JSONObject json = new JSONObject();
		List<Map> list = new ArrayList<>();
		List<CorrectMarkForm> listCorr =  new ArrayList<>();
		List<PshLackMarkForm> listLack =  new ArrayList<>();
		...
		if(userForm!=null&&userForm.getLoginName()!=null){
			...
			if(listLack!=null){
				List<Map> listLackMap = PshLackMarkConvertor.convertFormToMap(listLack);
			...
			list = ParamsToFrom.getListMapDescMarkTime(listMap);
			json.put("code", 200);
			json.put("data", list);
		...
		if(StringUtils.isNotBlank(reportType) && StringUtils.isNotBlank(objectId)){
			...
			if("add".equals(reportType)){
				...
				if(page.getResult()!=null&& page.getResult().size()>0){
					form = (PshLackMarkForm) page.getResult().get(0);
	...
	public String getFormId(String isCheck,CorrectMarkForm form) {
		...
		if(StringUtils.isNotBlank(form.getObjectId())){
			if("modify".equals(isCheck)|| "confirm".equals(isCheck)){
				...
				String hql="from PshLackMark ps where 1=1 and objectId="+form.getObjectId();
				List<PshLackMark> list = lackMarkDao.find(hql);
				if(list!=null&& list.size()>0)
					json.put("data", list.get(0).getId());
			}
			json.put("success", true);
	...
	private List<Object> getCountDay(Boolean yestDay,Map map){
		...
		String sql = "select PARENT_ORG_NAME,"+
			...
			"wm_concat(case typess when 'lack' then total else null end) lackCount "+
			 "from (select PARENT_ORG_NAME,count(*) as total,'lack' as typess from Lack_Mark ps where 1=1 "+ 
			 "and to_char(ps.mark_Time,'yyyyMMdd')=to_char(sysdate"+(yestDay?"-1":"")+",'yyyyMMdd') "+wheres+" GROUP BY PARENT_ORG_NAME UNION all "+
			 "select PARENT_ORG_NAME,count(*) as total,'corr' as typess from Correct_Mark ps where 1=1 "+
			 "and to_char(ps.mark_Time,'yyyyMMdd')=to_char(sysdate"+(yestDay?"-1":"")+",'yyyyMMdd') "+wheres+" GROUP BY PARENT_ORG_NAME ) GROUP BY PARENT_ORG_NAME";
		
		return correctMarkDao.getSession().createSQLQuery(sql).list();
	}
	/**
	 * 查看全部功能修改，数据源整合（事务公开功能）
	 * */
	public Object getPublicCountAll(Page pg,CorrectMarkForm form ,Map<String,Object> map,Class calzz){
		...
		StringBuffer sql = new StringBuffer("(select c.*,ROWNUM num from (select * from (select s.ID,s.MARK_PERSON,s.MARK_PERSON_ID,s.MARK_TIME,'correct' as source,"
			+"s.UPDATE_TIME,s.PARENT_ORG_ID,s.PARENT_ORG_NAME,s.SUPERVISE_ORG_ID,s.SUPERVISE_ORG_NAME,s.DIRECT_ORG_ID,s.DIRECT_ORG_NAME,s.ADDR,s.LAYER_NAME,s.CORRECT_TYPE from CORRECT_MARK s "
				+"UNION all select s1.ID,s1.MARK_PERSON,s1.MARK_PERSON_ID,s1.MARK_TIME,'lack' as source,"
					+"s1.UPDATE_TIME,s1.PARENT_ORG_ID,s1.PARENT_ORG_NAME,s1.SUPERVISE_ORG_ID,s1.SUPERVISE_ORG_NAME,s1.DIRECT_ORG_ID,s1.DIRECT_ORG_NAME,s1.ADDR,s1.LAYER_NAME,'数据确认' as CORRECT_TYPE from LACK_MARK s1 ) b "
	...
	public List<Map<String,Object>> countRoad(Map<String, Object> map){
		Map values = new HashMap<>();
		StringBuffer hql = new StringBuffer("select * from(select s.road from (select s.road,count(*) as total from Correct_Mark s where 1=1 ");
	 ...
	 * */
	@Override
	public PshLackMarkForm getLackForm(Long id) {
		return lackMarkService.get(id);
	}
	@Override
	public void save(PshLackMarkForm lackForm) {
