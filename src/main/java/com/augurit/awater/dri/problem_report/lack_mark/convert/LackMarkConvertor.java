...
public class LackMarkConvertor {
	public static LackMarkForm convertVoToForm(LackMark entity) {
		if(entity != null) {
	...
	public static void convertFormToVo(LackMarkForm form, LackMark entity) {
	...
	public static List<LackMarkForm> convertVoListToFormList(List<LackMark> lackMarkList) {
		if(lackMarkList != null && lackMarkList.size() > 0) {
			List<LackMarkForm> lackMarkFormList = new ArrayList();
			for(int i=0; i<lackMarkList.size(); i++) {
				lackMarkFormList.add(convertVoToForm(lackMarkList.get(i)));
			}
			return lackMarkFormList;
		}
		return null;
	}
	
	public static List<Map> convertVoListToMapList(List<LackMark> lackMarkList) {
		if(lackMarkList != null && lackMarkList.size() > 0) {
			List<Map> mapList = new ArrayList();
			for(int i=0; i<lackMarkList.size(); i++) {
				LackMark entity = lackMarkList.get(i);
				Map map = new HashMap();
	...
	public static List<LackMark> convertFormListToVoList(List<LackMarkForm> lackMarkFormList) {
		if(lackMarkFormList != null && lackMarkFormList.size() > 0) {
			List<LackMark> lackMarkList = new ArrayList();
			for(int i=0; i<lackMarkFormList.size(); i++) {
				LackMark lackMark = new LackMark();
				convertFormToVo(lackMarkFormList.get(i), lackMark);
				lackMarkList.add(lackMark);
			}
			return lackMarkList;
		}
		return null;
	 ...
	 * 移动端日期转成long
	 * */
	public static Map convertFormToMap(LackMarkForm form) {
		Map map = new HashMap();
	...
	public static List<Map> convertFormToMap(List<LackMarkForm> list) {
		List<Map> listMap = new ArrayList<>();
		for(LackMarkForm form :list){
			Map map = new HashMap();
			map.put("parentOrgId", form.getParentOrgId());
			map.put("parentOrgName", form.getParentOrgName());
			map.put("id", form.getId() == null ? "" : form.getId().toString());
