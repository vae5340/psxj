...
public class SynchronousDeleteData {
    ...
    //基于内存的阻塞队列  
    private static BlockingQueue<ReportDeleteForm> queue = new LinkedBlockingQueue<ReportDeleteForm>(QUEUE_LENGTH_DELETE);
    //创建保存计划任务执行器  
    private static ScheduledExecutorService deleteEsecute = null;  
    private static Boolean flag=false;
    
	public static IReportDeleteService reportDeleteService=null;
	@Autowired
	public SynchronousDeleteData(IReportDeleteService reportDeleteService){
		SynchronousDeleteData.reportDeleteService = reportDeleteService;
	}
	...
	private final static Runnable RunDeleteQueue = new Runnable() {
		...
		public void run() {
			try {
				//如果没有数据，则会阻塞线程
				ReportDeleteForm takse =  queue.take();
				Boolean flags =  httpArcgisClient.deleteFeature(takse.getObjectId());
				if(!flags){
					//删除失败需要定时器定时循环
					takse.setIsDelete("2");
					reportDeleteService.save(takse);
				}else{
	...
	public static void start(){
		if(deleteEsecute==null){
			...
			synchronized(deleteEsecute){
				...
				System.out.println("启动删除上报定时器任务成功!");
			}
		}
    }
	/**
		...
		if(deleteEsecute!=null){
			synchronized(deleteEsecute){
				if(deleteEsecute!=null && !deleteEsecute.isShutdown()){
					...
					System.out.println("关闭删除定时器任务成功!");
				}
			}
		}
	}
	 ...
	 * 添加数据到队列中
	 * */
	public static Boolean addFeature(ReportDeleteForm form){
		if(form!=null){
	 ...
	 * 添加数据到队列中
	 * */
	public static Boolean addDelete(ReportDeleteForm form){
		if(form!=null){
			return queue.offer(form);
		}else{
			return false;
	...
	public static void deleteFeature(){
		synchronized(SynchronousUpdateData.class){
			List<ReportDeleteForm> list =  reportDeleteService.getDeleteStatus("2");
			int i=0;
			if(list!=null && list.size()>0){  
				for(ReportDeleteForm form : list){
					if(SynchronousDeleteData.addDelete(form)){
						i++;
					}
				}
			}
