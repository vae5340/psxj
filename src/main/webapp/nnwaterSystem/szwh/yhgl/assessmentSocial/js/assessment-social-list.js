...
var FileList = null;
$(function() {
	$("#table").bootstrapTable({toggle:"table", url:"/agsupport/sz-assessmentsocial!listJson.action", 
	rowStyle:"rowStyle", 
...
function reloadData(){
	...
	'refresh', {
		url:"/agsupport/sz-assessmentsocial!listJson.action",query:query
	});     
}

function addBtnCol(value, row, index){
	  return "<button id=\"btn_edit\" class=\"btn btn-primary\" style=\"border:1px solid transparent;\" onclick=\"detailDialog("+row.assessmentSocialId+")\"><span class=\"glyphicon\" aria-hidden=\"true\"></span>详细</button>";
}
...
function addData(){
   	layer.open({
		...
		maxmin: true, 
		area: ['900px','550px'],
	    content: 'assessment-social-edit.html'
	});
}
 	...
 	if(selList.length==0)
 		layer.msg('请选中删除项');
	  	...
	  	for (var i=0;i<selList.length;i++){
	  		checkedIds.push(selList[i].assessmentSocialId);
	  	}
 	    $.ajax({  
		    url: location.protocol+"//"+location.hostname+":"+location.port+'/agsupport/sz-assessmentsocial!deleteMoreJson.action',  
		    traditional: true,
...
function detailDialog(id){
	layer.open({
			...
			maxmin: true, 
			area: ['900px','550px'],
		    content: 'assessment-social-edit.html?id='+id
	});
}
