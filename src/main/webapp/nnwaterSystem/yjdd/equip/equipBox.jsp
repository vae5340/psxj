...
<li>  
      ...
      <ul>
     	<w:iterate id="team" bind="#info.equipList">
     	<w:define id="tid" bind="#team.id"/>
     	<%boolean bl=false;if(ss!=null){for(String s:ss){if(s.equals(tid.toString()))bl=true;}} %>
      	<li id="<w:write bind="#team.id"/>" data-options="iconUrl:'<%=path %>/resources/images/icons/16_16/application_group.png',checked:<%=bl %>"><span><w:write bind="#team.name"/>（型号:<w:write bind="#team.model"/>&nbsp;&nbsp;&nbsp;&nbsp;权属单位:<w:write bind="#team.ownerdept"/>）</span></li>
      	</w:iterate>
      </ul>
</li>
</w:iterate>
