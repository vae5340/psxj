...
<html xmlns="http://www.w3.org/1999/xhtml">
   ...
   $(function(){
   	$("#panteamTree").tree({
	   	...
	   	onClick:function(node){
	   		var did=node.id;
   			$w.execUrl(ctx+"/gzps/yjdd/GoodPage@equipList.page?bean.type="+did,{},"panInfopanel");
   			
	   	}
   	})
   	$w.execUrl(ctx+"/gzps/yjdd/GoodPage@equipList.page",{},"panInfopanel");
   })
  
  function addsptype(){
  	window.$w.openDialog({title:'添加物资分类',url:ctx+"/gzps/yjdd/GoodPage@typeShow.page?new_=true",w:300,h:180,afterClose:function(re){
		if(re){
  ...
  function editsptype(){
  	var typeid="";
   	...
   	if(typeid){
	   		window.$w.openDialog({title:'修改物资分类',url:ctx+"/gzps/yjdd/GoodPage@typeShow.page?new_=false&id="+typeid,w:300,h:180,afterClose:function(re){
				if(re){
  ...
  function delsptype(){
  	var typeid="";
   	...
   	if(typeid){
		   	...
		   	if(r){
		   		$w.doUrl(ctx+"/gzps/yjdd/GoodPage@delType.page?id="+id,{},function(){
  ...
  function addUser(type){
  	window.$w.openDialog({title:'添加物资',url:ctx+"/gzps/yjdd/GoodPage@addShow.page?bean.type="+type,w:400,h:400,afterClose:function(re){
			if(re){
  ...
  function editUser(id){
  	window.$w.openDialog({title:'修改物资',url:ctx+"/gzps/yjdd/GoodPage@updateShow.page?id="+id,w:400,h:400,afterClose:function(re){
			if(re){
					$w.execForm({target:'panInfopanel'})
			}
		}});
   ...
   <body class="easyui-layout" fit="true">
    <div data-options="region:'west',split:true,border:false" style="width:170px">
    	<div class="easyui-panel" fit="true" title="应急物资分类" style="">
    ...
    <div data-options="region:'center',border:false">
		<div class="easyui-panel" fit="true" title="应急物资信息" id="panInfopanel">
