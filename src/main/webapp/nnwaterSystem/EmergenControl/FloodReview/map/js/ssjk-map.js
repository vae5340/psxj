	...
	require([
		...
		if(map.getLayer(featureLayreId)){
            ...
            featureLayer.on("mouse-over", function (g) {
			    ...
			    var extent = new esri.geometry.Extent({"xmin":pt.x-radius,"ymin":pt.y-radius,"xmax":pt.x+radius,"ymax":pt.y+radius,"spatialReference":{wkid:2433}});
...
function addAllMonitorStation(result){
	...
	for(var i=0;i<rows.length;i++){
		var row=rows[i];
		var combId=row.comb_id;
		var combName=row.comb_name;
		var estType=row.est_type;
		var x=row.xcoor;
		var y=row.ycoor;
		var laneWay=row.lane_Way;
...
function showJsdDispatch(g){
	parent.layer.open({
		  ...
		  shade: 0,
		  area: ['700px', '440px'],
		  //content: "/awater/nnwaterSystem/EmergenControl/District/map/ssjk-jsd.html?combId="+g.graphic.attributes.combId+"&recordId="+recordId
		  content:"/awater/nnwaterSystem/EmergenControl/District/Template/jsdDispatch.html?combId="+g.graphic.attributes.combId+"&modelId="+recordId+"&yaType="+2+"&type="+4
		}); 
}
