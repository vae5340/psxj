...
function formatter_overView(value,row,index){
	if(row.recordStatus==1)
	    return "<button class='btn btn-primary'  onclick=showNowTabWindow('"+row.id+"')>详情</button>";
	else
		return "<button class='btn btn-primary'  onclick=showHistoryTabWindow('"+row.id+"')>详情</button>";
}
//格式化查看报告按钮-进入报告汇总页面
