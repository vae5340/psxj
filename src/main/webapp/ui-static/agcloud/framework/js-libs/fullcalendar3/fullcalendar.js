...
function mergeProps(propObjs, complexProps) {
	...
	for (i = propObjs.length - 1; i >= 0; i--) {
		...
		for (name in props) {
			if (!(name in dest)) { // if already assigned by previous props or complex props, don't domain
...
DayGrid.mixin({
	...
	//TODO
	// Builds the HTML to be used for the default element for an individual segment  月视图，日视图
	fgSegHtml: function(seg, disableResizing) {
		var view = this.view;
		var event = seg.event;
		var isDraggable = view.isEventDraggable(event);
		...
		/*if(event.isTaskConfirm!='undefind' &&event.isTaskConfirm!=null){
			console.log(event.isTaskConfirm);
		}*/
		titleHtml =
			'<span class="fc-title">' +
				( (event.projName || '') || '&nbsp;') + // we always want one line of height
			'</span>'+'<div class="fc-ico">' +
			(event.isTaskConfirm=='1'?'<img src="' +ctx+
			'/erp/themes/proj/images/03.png">'
				:event.isTaskConfirm=='2'?'<img src="' +ctx+
			'/erp/themes/proj/images/01.png">':event.isTaskConfirm=='3'?'<img src="' +ctx+
			'/erp/themes/proj/images/02.png">':'')
			+ '</div>' ;
			...
			'>' +
				'<div class="fc-content">' +
					(this.isRTL ?
						...
						timeHtml + ' ' + titleHtml+'<br/><div class="fc-title01">'+ event.taskContent +'</div>' //
						) +
				'</div>' +
...
TimeGrid.mixin({
	...
	// Renders the HTML for a single event segment's default rendering  周视图 TODO
	//修改源代码
	fgSegHtml: function(seg, disableResizing) {
		var view = this.view;
		var event = seg.event;
		...
		return '<a class="' + classes.join(' ') + '"' +
			...
			'>' +
				'<div class="fc-content1">' +
					(timeText ?
						'<div class="fc-time"' +
						' data-start="' + htmlEscape(startTimeText) + '"' +
						' data-full="' + htmlEscape(fullTimeText) + '"' +
						'>' +
							'<span>' + htmlEscape(timeText) + '</span><br/>' +
						'<span class="fc-title01">'
						+event.projName+ '</span>'+
						'</div>' :
						''
						) +
					(event.title ?
						'<div class="fc-title01">' +
							(event.taskContent) +
						'</div>' :
						''
						) +
