		...
		    //pageList: [10, 25, 50, 100],
			...
			onLoadSuccess:function(data){
				...
				layer.open({
                    ...
                    content: ['/awater_swj/psh/psh_sh/showSq/psh_checkInput.html?id='+NoTable.current.id, 'yes'],
...
//				{title:'操作',visible:true,align:'center',formatter: format_oper}
				...
				onLoadSuccess:function(data){
					...
					layer.open({
                        ...
                        title : "查看详情",
                        btn:['上一条','下一条','存疑','返回'],
						content: ['/awater_swj/psh/psh_sh/showSq/psh_checkInput.html?id='+row.id, 'yes'],
                        btn1: function(index,layero){
                            var iframes = $(layero).find("iframe")[0].contentWindow;
                            if(HasTable.getData){
...
function toCheckRecall(thisId){
    if(thisId){
        $.ajax({
            type:'post',
            url:'/agsupport_swj/nw-check-record!recallCheck.action',
            data:{id:thisId},
            dataType:'json',
