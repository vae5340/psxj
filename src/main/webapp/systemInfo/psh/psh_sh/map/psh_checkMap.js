...
function pshInfo(mpId){
	layer.open({
		...
		maxmin: true,
		btn:['确定','取消'],
		content: ['/awater_swj/psh/psh_query/mpToPshList.html?mpId='+mpId, 'yes']
	});
}
//地图初始化  结束
//点击查看数据
...
function orgLoadMap(){
    ...
    /*$.ajax({
       type:'get',
        url:'/agsupport_swj/nw-check-record!getUserOrgs.action',
        dataType:'json',
    ...
    ToCheckInput.InitHtml = function(markid){
        if(markid){
            ...
            layer.open({
                ...
                title : "查看详情",
                btn:['上一条','下一条','返回'],
                content: ['/awater_swj/psh/psh_sh/showSq/psh_checkInput.html?id='+markid, 'yes'],
