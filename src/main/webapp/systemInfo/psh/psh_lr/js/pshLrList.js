...
//是否显示排水户导入,判断用户是否为系统管理员，是则显示，否则不显示
function isShowImport(){
	$.ajax({
		type : 'get',
		url: '/agsupport_swj/asi/facilitymgr/facilitymgr/uploadfile!userIsAdmin.action',
		dataType: 'json',
...
function importPsh(){
	layer.open({
		...
		maxmin: true,
		btn:['导入','取消'],
		content: ['/awater_swj/psh/psh_lr/pshImport.html', 'yes'],
...
function updateShow(id){
	layer.open({
		...
		maxmin: true,
		btn:['保存','取消'],
		content: ['/awater_swj/psh/psh_lr/pshLrInput.html?type=update&id='+id, 'yes'],
		btn1: function(index,layero){
...
function deleteRow(thisId){
	layer.confirm('确定删除？', {
    	...
    	$.ajax({
            type: 'post',
            url: '/agsupport_swj/psh-discharger!deleteRow.action',
            data: {id: thisId},
            dataType: 'json',
            success: function (data) {
            	if(data.success){
