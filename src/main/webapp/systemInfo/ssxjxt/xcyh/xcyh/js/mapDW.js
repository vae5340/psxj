...
function addToMap(evt){
	...
	// 赋值
    /*$(parent.frames["myform"]).find("#x").val(x)
    $(parent.frames["myform"]).find("#y").val(y)*/
    //parent.frames["myform"].$("#X").val(x);
    //parent.frames["myform"].$("#Y").val(y);
    parent.$("#X").val(x);
    parent.$("#Y").val(y);
	setLocation(x, y);
	// 关闭
...
　　　　　　var wtdd=addComp.province  + addComp.city + addComp.district + addComp.street +  addComp.streetNumber;
       ...
       /*$(parent.frames["myform"]).find("#szwz").val(wtdd);
       $(parent.frames["myform"]).find("#jdmc").val(addComp.street);*/
	});
}
...
//管网
function createGW(){
	 var url_=window.top.awater.url.url_h;
