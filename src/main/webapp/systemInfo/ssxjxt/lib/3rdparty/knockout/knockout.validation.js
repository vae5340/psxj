...
*/
/***
 * github: https://github.com/ericmbarnard/Knockout-Validation
 */
(function (factory) {
    ...
    var defaults = {
        ...
        errorShow: 'inBubble',  //add by lcy,默认错误显示方式
        errorShowContext: null,
    ...
    var api = (function () {
        ...
        return {
            ...
            insertValidationMessage: function (element, obs) {
               /* var span = document.createElement('SPAN');
                span.className = utils.getConfigOptions(element).errorMessageClass;
                if ( configuration.insertFn ) {
                	configuration.insertFn(span);
                } else {
                    utils.insertAfter(element, span);
                }
                return span;*/
            	return validation.errorShows[configuration.errorShow].createElement(element,configuration.errorShowContext,obs);
            },
    ...
    /***
     * add by lcy
     * 自定义错误信息显示，源码只提供了在元素后面插入span来显示
     */
    validation.errorShows = {};
    
    /***
     * 源码本来提供的方式，提取出来，作为一种可选的错误显示方式，也是默认的方式
     */
    validation.errorShows['insertSpan'] = {
    	/***
    	 * 生成错误显示的元素
    	 * element: 正在检验的dom元素
    	 * context: 执行环境，有用户配置
    	 * 返回错误信息的dom元素
    	 */
    	createElement: function(element,context) {
    		var span = document.createElement('SPAN');
            span.className = utils.getConfigOptions(element).errorMessageClass;
            utils.insertAfter(element, span);
            return span;
    	...
    	/***
    	 * 错误信息更新回调
    	 * element 错误显示元素，即生成的span
    	 * errorMsgAccessor 错误信息读取函数
    	 * visiblityAccessor 可见信息读取函数
    	 */
        ...
        return {
            init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
                ...
                if (config.insertMessages && utils.isValidatable(valueAccessor())) {
                    ...
                    var validationMessageElement = exports.insertValidationMessage(element,valueAccessor());
    ...
    ko.bindingHandlers['validationMessage'] = { // individual error message, if modified or post binding
        update: function (element, valueAccessor) {
             ...
             * add by lcy
             */
            var errorShow = $(element).data('errorShow') || defaults.errorShow;
            if ( validation.errorShows[errorShow].updateError ) {
            	validation.errorShows[errorShow].updateError(element,errorMsgAccessor,visiblityAccessor);
            } else {
            	ko.bindingHandlers.text.update(element, errorMsgAccessor);
                ko.bindingHandlers.visible.update(element, visiblityAccessor);
            }
    ...
    function validateSync(observable, rule, ctx) {
        ...
        if (!rule.validator(observable(), ctx.params === undefined ? true : ctx.params)) { // default param is true, eg. required = true
        	...
        	//add by lcy,增加消息前缀
            var msg = (observable.msgPrefix || '') + exports.formatMessage(ctx.message || rule.message, ctx.params);
            observable.error(msg);
            observable.__valid__(false);
            return false;
        } else {
            return true;
    ...
    validation.validateObservable = function (observable) {
    	if ( configuration.isSubmit == false ) {
        	if ( configuration.showError == false || (validation.isCalledByUser && !validation.isCalledByUser())) {
        		observable.error(null);
                observable.__valid__(true);
                return true;
        	}
