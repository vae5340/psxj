; (function($) {
	$.simpleCanleder = function(box, options){
		...
		function _init(){
			...
			if(box.text()){
				year = box.text().split("-")[0] * 1;
				month = box.text().split("-")[1] * 1;
			}
		...
		function _buildBody(canlederBox){
			...
			for(var i = 0; i < 12; i++){
				...
				var $li = $("<li/>").append($inner).click(function(){
					...
					canlederBox.data("box").html(year + "-" + month);
					canlederBox.hide();
					options.updateDay(year,month);
				});
    ...
    $.fn.extend({
        simpleCanleder: function(options) {
            options = $.extend({updateDay: function(year,month){}},options);
            this.each(function() {
