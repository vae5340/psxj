...
//eUrban code yindl
if(tt.hasClass("tree-radio")){
	tt.parents(".tree").find(".tree-radio").removeClass("tree-radio1").addClass("tree-radio0");
	tt.addClass("tree-radio1");
}
...
// eUrban code yindl
function getRadio(_yy){
	return _c(_yy, $(_yy).find("span.tree-radio1").parent()[0]);
};
...
},getRadio:function(jq){
	return getRadio(jq[0]);
...
//eUrban code  yindl
if(opts.radio){
	if((!opts.onlyLeafRadio)||_120){
		cc.push("<span class=\"tree-radio tree-radio0\"></span>");
	}
}
...
//urban code  yindl: radio:false,onlyLeafRadio:true
$.fn.tree.defaults={url:null,method:"post",radio:false,onlyLeafRadio:true,animate:false,checkbox:false,cascadeCheck:true,onlyLeafCheck:false,lines:false,dnd:false,data:null,queryParams:{},formatter:function(node){
return node.text;
