var myChars,option,layerIds,tableId,rowDatas=[],tables=["table_all","table_yj","table_ysk","table_pfk","table_bz","table_pm","table_fm","table_yly"],
	url="",seriesDatas;
$(function(){
	//初始化点击事件
	btn_click();
...
function initMyEchars(){
	if(options){
		option = options;
		option.title.text="数据新增统计";
		option.series[0].name="数据新增统计";
...
function initTable(tableId){
	$("#"+tableId).bootstrapTable({
		...
		columns:[
			...
			{field:'REPAIR_COM',title:'采集人',visible:true,align:'center'},
			{field:'REPAIR_DAT',title:'调查日期',visible:true,align:'center',formatter: format_Data},
			{field:'SORT',title:'类别',visible:true,align:'center'},
			{field:'FLAG_',title:'状态',visible:true,align:'center', formatter: format_flaag},
...
function format_ck(value,row,index){
	var a = "<button type=\"button\" class=\"btn btn-primary btn-sm\"" +
		"onclick=\"openMapToAlertDatas('"+row.OBJECTID+"')\">" +
		"详情</button>&nbsp;";
	if(!row.x && layerIds<4){
		return a;
	}else if(row.x>0 && row.y>0){
		return a+="<button type=\"button\" class=\"btn btn-primary btn-sm\"" +
...
function format_flaag(val,row,index){
	var add = '<font color="red">新增</font>';
	var update = '<font color="blue">修正</font>';
	if(val)
		return val=='1'? update:(val=='2'? add:'');
}
...
function loadMaptoQuery(){
	...
	var ik=0;
	var where = "1=1 and FLAG_ is not null and REPAIR_DAT is not null";// and FLAG_ is not null
...
function parentMethod(layerId){
	layerIds=layerId;
	var where = "1=1 and FLAG_ is not null and REPAIR_DAT is not null";
	window.parent.querysData("",layerId,where,quertResult);
}
//父页面详细信息
function openMapToAlertDatas(objectid){
	window.parent.alertDatas(window.parent.gwurl_bc,layerIds,objectid);
}
function loadDataTable(tableId,data){
	data.sort(sortTime);
	rows=[];
	for(var i=0;i<data.length;i++){
		if(data[i]) rows[i] = data[i];
	}
