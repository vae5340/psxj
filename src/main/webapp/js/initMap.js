...
var gwurl_1=awater.url.url_h,gwurl_bc=awater.url.url_nc,qsfw_bj=awater.url.url_qsfw_bj,qsfw_tc=awater.url.url_qsfw_dt,
	...
	pshsb_tc=awater.url.url_pshsb,gwurl_2,gwurl_3,feedback_tc=awater.url.url_feedback;
...
//地图初始化
function initMap() {
    esri.config.defaults.io.proxyUrl= "/psxj/esri_proxy.jsp";
...
//加载示意连线图层
function showSylxTc(){
	var featureLayerSylx = new esri.layers.ArcGISDynamicMapServiceLayer(sylx_tc, {id: 'sylx_tc',visible:false});
	map.addLayer(featureLayerSylx);
}
//应开未开井图层
function showYkwkjTc(){
	var featureLayerYkwkj = new esri.layers.ArcGISDynamicMapServiceLayer(ykwkj_tc, {id: 'ykwkj_tc',visible:false});
	map.addLayer(featureLayerYkwkj);
}
//排水户上报分布图
function showPshsbTc(){
	var featureLayerPshsb = new esri.layers.ArcGISDynamicMapServiceLayer(pshsb_tc, {id: 'pshsb_tc',visible:false});
	map.addLayer(featureLayerPshsb);
}
function report_data(){
     layerIdReport = new esri.layers.ArcGISDynamicMapServiceLayer(awater.url.url_report[0],{id:awater.url.url_report[1],visible:false});
     var layer = layerIdReport;
     return layer;
}
//暂不用 第一步  勾选以后，先弹出清单列表
function qdList(){
	layer.open({
		type: 2,
		area: ['800px', '400px'],
		title : "交办清单",	
		maxmin: true,
		content: ['/psxj/systemInfo/ssxjxt/problem_report/assign_feedback/qdListForMap.html?loginName='+userCode, 'yes'],
		end: function () {//关闭窗口后，取消选中
//			$("#checkbox_feedback_tc").prop("checked",false);
			//先关闭掉弹框
//			layer.closeAll();
        }
	});
...
//第一步，显示反馈图层
function showInitFeedBackTc(){
	var featureLayerFeedBack = new esri.layers.ArcGISDynamicMapServiceLayer(feedback_tc, {id: 'feedback_tc'});
	map.addLayer(featureLayerFeedBack);
	map.on('click',doIdentifyForFeedBack);
}
//第二步，在弹出的列表中，选择批次，在展示对应反馈交办分布图
function showFeedBackTc(qdbh){
	var layerMap = map.getLayer("feedback_tc");//获取图层
	if(typeof(layerMap)=="undefined"){//如果一开始没有图层，就加载一下
		showInitFeedBackTc();
		layerMap=map.getLayer("feedback_tc");
	}
	var layerDefinitions=[];
	layerDefinitions[0] = "QDBH="+qdbh ; //
	layerMap.setLayerDefinitions(layerDefinitions);//过滤一下当前批次	
	layerMap.refresh();//刷新一次
	$("#checkbox_feedback_tc").prop("checked",true);//勾选上
}
//第三步：点击事件 反馈交办点击，显示出详细信息
function doIdentifyForFeedBack(event) {
    ...
    var layerMap = map.getLayer("feedback_tc");
    if(!layerMap || !layerMap.visible)
		return;
    //开始按范围查询所点击区域的点
    var identifyTask = new esri.tasks.IdentifyTask(feedback_tc); 
    var identifyParams = new esri.tasks.IdentifyParameters();// 查询参数
    identifyParams.tolerance = 10;// 容差范围
    identifyParams.returnGeometry = true;// 是否返回图形
    identifyParams.layerIds = [0];// 查询图层 有多少个图层就写几个
    identifyParams.layerOption = esri.tasks.IdentifyParameters.LAYER_OPTION_VISIBLE;// 设置查询的图层
    // 查询范围
    identifyParams.width = map.width;  
    identifyParams.height = map.height;  
    ...
    identifyTask.execute(identifyParams,function(results){
    	...
    	var texts="<div style=\"overflow-x: hidden;\"><ul class=\"meumul\">";
    	for ( var i = 0; i < results.length; i++) {
    		var result= results[i].feature.attributes;//获取信息feature
 		  	var objectId=result.OLDOID;
 		  	var qdzt =result.QDZT;
 		  	if(!qdzt){
 		  		qdzt =result.qdzt;
 		  	}
 		  	if(qdzt =="1"){
 		  	...
 		  	var d =new Date(result.mark_time)
 		  	var t = getLocalTime(d.getTime()+8*60*60*1000);
 		    texts+="<li class=\"meumli\" onclick=\"feedBackInfo('"+result.report_type+"','"+result.mark_id+"')\">" 
 		  	+ "<span>&nbsp;交办状态：</span>" + qdzt  + "<br>"  
 		  	+ "<span>&nbsp;设施类型：</span>" + result.layer_name  + "<br>"  
            + "<span>上报人：</span>" + result.mark_person  + "<br>"  
            + "<span>上报时间：</span>"  + t  + "<br>"
            + "<span>上报单位：</span>"  + result.parent_org_name  + "<br>"  
            + "<span>地址：</span>"  + result.addr +"</li>";
    	 }	
 		texts+="</ul></div>";
	  	map.infoWindow.hide();
    	map.graphics.clear();  
    	map.infoWindow.setTitle("交办设施信息"); 
...
function feedBackInfo(type,mid){
	//这里判断一下再跳转
	var url,weigt,hight;
		...
		btn1: function(index, layero){
			...
			layer.open({
				...
				btn1: function(index,layero){
						...
						jsonString = jsonString + ',"SBR":"' + $("#userName").text() + '"';
			                ...
			                success : function (json) {
			                    ...
			                    if(json && json.save){
			                    	if(json.save=='success_self'){//自行处理
			                    		parent.layer.alert("自行处理成功！可到“问题列表”中查看",function(){
					                		 parent.layer.closeAll();
				                        });
			                    	}else{
			                    		parent.layer.alert("转办成功！请到“问题上报”-“待受理”中查看",function(){
					                		 parent.layer.closeAll();
				                        });
			                    	}
			                	 }
...
//加载门牌图层
function showMpTc(){
	var featureLayerMp = new esri.layers.ArcGISDynamicMapServiceLayer(mp_tc, {id: 'mp_tc',visible:true});
...
function isShowEdit(){
	$.ajax({
		...
		// url: '/' + serverName + '/gx-problem-report!userIsAdmin.action',
		 url: '/psxj/data/awater/userIsAdmin.json',
		dataType: 'json',
		success : function(data){
			if(data.success){
				$("#qsfwBts").show();
...
function initSelectToolbar() {
    ...
    drawToolbar.on("draw-end", function(evt) {
        ...
        identifyTask.execute(identifyParams,function(results) {
        	...
        	$.ajax({//获取当前用户所在区域
        		type : 'get',
        		url: '/' + serverName + '/gxProblemrReport/getUserSzqy',
        		dataType: 'json',
        		success : function(data){
        			if(data){
        			    if(!data.success){
...
function isShowGxOrWtsb(){
	var cotents='<div style="width: 100%;height: 80%;">'
				...
				+'排水户上报'
				+'</br>'
				+'<input type="checkbox" class="styled" id="checkbox_feedback_tc" onclick="isShowDate(this.id)"/>'
				+'交办设施'
				+'</div>';
...
function isShowDate(val2){
	...
	if($("#"+val2).is(':checked')) {
		...
		if(val2.indexOf("feedback_tc")!=-1 && typeof(layerMap)=="undefined"){//交办设施数据，如果没加载就执行一下
			showInitFeedBackTc();//加载一下
			layerMap = map.getLayer(val);//再获取一次
		}
...
function getWtsbByLoginName(){
	$.ajax({
		...
		url: '/psxj/data/awater/getByLoginName.json',
...
function createGW(gwurl_1){
   ...
   /* //layer.setMaxScale(11);
	...
	if(!parentOrgId){
		...
		//获取当前登录账户，这个隐藏域在index.html中，填值方法在common.js中
...
//地图打印查看打印范围
function positionArea(xMin , xMax , yMin , yMax){
	var extent=new esri.geometry.Extent(xMin,yMin,xMax,yMax,map.spatialReference);
	map.graphics.clear();
    var centerPoint = extent.getCenter();//获取多边形中心点  
...
require(["dojo/ready"], function (ready) {
    ready(function () {
    	$.ajax({
			...
			url:'/psxj/data/awater/showAreaData.json',
			dataType: 'json',
...
var TrackPlayback = function(){
    var TrackPlayback = new Object();
    TrackPlayback.init = function(id,callback){
        TrackPlayback.list,
        TrackPlayback.icount,
        TrackPlayback.pointSymbol,
        TrackPlayback.symbolLine,
        TrackPlayback.inum=0;
        $.ajax({
            method : 'get',
            url:'/psxj/trackPoint/getFormByTrackId',
            data:{"trackId":id},
            dataType: 'json',
            success : function(data){
                if(data.code=="200" && data.data && data.data.length>0){
                    map.graphics.clear();
                    var pointSymbol0 = new esri.symbol.SimpleMarkerSymbol();
                    pointSymbol0.setOutline = new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_SOLID, new dojo.Color([255, 0, 0]), 1);
                    pointSymbol0.setSize(5);
                    pointSymbol0.setColor(new dojo.Color([0, 255, 0, 0.25]));
                    var geometry= new esri.geometry.Point(data.data[0].x, data.data[0].y,map.spatialReference);
                    var graphic= new esri.Graphic(geometry, pointSymbol0);
                    map.graphics.add(graphic);
                    map.centerAndZoom(geometry, 10);
                    TrackPlayback.list =data.data;
                    TrackPlayback.icount=TrackPlayback.list.length;
                    TrackPlayback.inum++;
                    TrackPlayback.pointSymbol = new esri.symbol.SimpleMarkerSymbol();
                    TrackPlayback.pointSymbol.setOutline= new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_SOLID,new dojo.Color([255, 0, 0]), 1);
                    TrackPlayback.pointSymbol.setSize(5);
                    TrackPlayback.pointSymbol.setColor(new dojo.Color([0, 255, 0, 0.25]));
                    TrackPlayback.symbolLine = new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_DASHDOTDOT, new dojo.Color([255,0,0]), 2);
    ...
    TrackPlayback.go = function(){
       /* if(!this.list[this.inum] || !this.list[this.inum].x || !this.list[this.inum].y){
            this.inum++;
            console.log(this.list[this.inum]);
            return false;
        }*/
        //画点
        var geometry= new esri.geometry.Point(this.list[this.inum].x,this.list[this.inum].y,map.spatialReference);
        var graphic = new esri.Graphic(geometry, this.pointSymbol);
        map.graphics.add(graphic);
        ...
        //画线
        var polyline = new esri.geometry.Polyline([[this.list[this.inum-1].x,this.list[this.inum-1].y],[this.list[this.inum].x,this.list[this.inum].y]]);
        var graphicLine = new esri.Graphic(polyline, this.symbolLine);
        map.graphics.add(graphicLine);
        this.inum++;
    },
    TrackPlayback.gogps=function(){
        this.go();
        if (this.inum <this.icount) {
           this.gogps();
        }
    }
