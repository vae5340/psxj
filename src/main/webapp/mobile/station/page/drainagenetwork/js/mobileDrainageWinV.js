...
function initData() {
    ...
    $.ajax({
        ...
        success: function(result){
            ...
            $("#drainageWinVFromDate").val(getPastDate(maxDate,1));
            $("#drainageWinVToDate").val(maxDate);
            $("#drainageWinVFromDate").click(function () {
                WdatePicker({
                    ...
                    minDate: minDate,
                    maxDate: "#F{$dp.$D('drainageWinVToDate')}"
                });
            });
            $("#drainageWinVToDate").click(function () {
                WdatePicker({
                    onpicked: function (dp) {
                        clickSearchBtn(true);
                    },
                    dateFmt: 'yyyy-MM-dd HH:mm:ss',
                    minDate: "#F{$dp.$D('drainageWinVFromDate')}",
                    maxDate:'%y-%M-%d %H:%m:%s'
                });
            });
        }
...
function getHisData() {
    ...
    var fromDate = $('#drainageWinVFromDate').val();
    var toDate = $('#drainageWinVToDate').val();
...
function loadHisChart(data,isRealTime) {
    ...
    //标题颜色配置的对象
    var objConfig ={
        "normal":{title:"流速",filed:"z",color:"#90FCFC"},
        "avg":{title:"平均流速",filed:"avg",color:"#6bc6fb"},
        "max":{title:"最大流速",filed:"max",color:"#fab51b"},
        "min":{title:"最小流速",filed:"min",color:"#1ab382"},
        "level1":{title:"警戒流速1",filed:"gdsd",color:"red"},
        "level2":{title:"警戒流速2",filed:"yjsd",color:"orange"},
        "level3":{title:"警戒流速3",filed:"jjsw",color:"yellow"}
    };
    ...
    if (data) {
        for (var i = data.content.length-1; i >=0; i--) {
            ...
            warnArr.push(0.03);
            if(isRealTime) {
                maxArr.push(data.content[i].maxV);
                minArr.push(data.content[i].minV);
                avgArr.push(data.content[i].avgV);
            }else {
                dataArr.push(data.content[i].v);
            }
        }
    }
    ...
    if(isRealTime){
        ...
        pushChartData('过低流速',[0.03],'red',true);
    }
