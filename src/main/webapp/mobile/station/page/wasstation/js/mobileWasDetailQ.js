...
function initData() {
    ...
    $.ajax({
        ...
        success: function(result){
            ...
            $("#wasDetailQFromDate").val(getPastDate(maxDate,1));
            $("#wasDetailQToDate").val(maxDate);
            $("#wasDetailQFromDate").click(function () {
                WdatePicker({
                    ...
                    minDate: minDate,
                    maxDate: "#F{$dp.$D('wasDetailQToDate')}"
                });
            });
            $("#wasDetailQToDate").click(function () {
                WdatePicker({
                    onpicked: function (dp) {
                        clickSearchBtn(true);
                    },
                    dateFmt: 'yyyy-MM-dd HH:mm:ss',
                    minDate: "#F{$dp.$D('wasDetailQFromDate')}",
                    maxDate:'%y-%M-%d %H:%m:%s'
                });
            });
        }
...
function getHisData() {
    ...
    var fromDate = $('#wasDetailQFromDate').val();
    var toDate = $('#wasDetailQToDate').val();
...
function loadHisChart(data,isRealTime) {
    ...
    if (data) {
        for (var i = data.content.length-1; i >=0; i--) {
            ...
            //数据取得字段有问题                      
            if(isRealTime){
                avgQData.push(data.content[i].tgtq);
                maxQData.push(data.content[i].maxTgtq);
                minQData.push(data.content[i].minTgtq);
            }else{
                qData.push(data.content[i].tgtq);
            }
        }
    }
