...
function reloadDate(myId){
	...
	if(myId)
		$.ajax({
			method: 'get',
			url: '/psxj/pshDischarger/seePsh?id='+myId,
			dataType:'json',
				...
				if(data.code==200 && data.rows && data.rows.length>0){
					for(var i in data.rows){
						...
						if(rowData.mime.indexOf('image')!=-1 || rowData.mime.indexOf('png')!=-1){
							if(CheckImgExists(rowData.attPath)){
                                imgUrl = rowData.attPath;
                                thumPath=rowData.thumPath;
							}else{
								var imageSrc ="http://139.159.243.185:8080";
                                if(CheckImgExists(imageSrc+rowData.attPath)){
                                    imgUrl = imageSrc+rowData.attPath;
                                    thumPath=imageSrc+rowData.thumPath;
								}else{
                                    var imageSrc ="http://139.159.243.230:8080";
                                    if(CheckImgExists(imageSrc+rowData.attPath)) {
                                        imgUrl = imageSrc + rowData.attPath;
                                        thumPath = imageSrc + rowData.thumPath;
                                    }
...
function checkRecord(){
	...
	if(id){
		$.ajax({
			method: 'get',
			url: '/psxj/pshCheckRecord/searchCheckRecord',
...
var CheckRecode=function(){
    ...
    CheckRecode.updatePass=function(row,index){
        ...
        var url="/psxj/correctMark/updateForm";
        if(url!=""){
            $.ajax({
                type : 'post',
                url : url,
...
function updateForm(){
        ...
        if(checkForm){
            $.ajax({
                type : 'post',
                url:'/psxj/pshDischarger/toUpdateForm',
                data:checkForm,
    ...
    if(state) {
        parent.layer.open({
            ...
            yes: function (index, layero) {
                ...
                $.ajax({
                    type : 'post',
                    url:'/psxj/pshCheckRecord/saveCheckInf',
                    data: data,
                    dataType: 'json',
                    success: function (data) {
                        if(data.code=='200'){
...
function updateSb(state){
	...
	if(state=='3'){
     ...
     if (checkForm) {
         $.ajax({
             type: 'post',
             url: '/psxj/pshDischarger/checkDate?shState='+state,
             data: checkForm,
             contentType: 'application/x-www-form-urlencoded',
             dataType: 'json',
             success: function (data) {
