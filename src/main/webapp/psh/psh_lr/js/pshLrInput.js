...
function initFileInput(){
	$("#xctpFile").fileinput({
        language: 'zh', //设置语言
        uploadUrl: "/psxj/asi/facilitymgr/facilitymgr/uploadfile/uploadFileForPsh", //上传的地址
...
function reloadData(){
	if(id)
		$.ajax({
			method: 'get',
			url: '/psxj/pshDischarger/seePsh?id='+id,
			dataType:'json',
			async:false,
				...
				if(data.code==200 && data.form){
					...
					if(data.yyzz!=null && data.yyzz.length>0){
						for(var i in data.yyzz){
	            			...
	            			config.url="/psxj/sewerageUserAttachment/deleteById";
	...
	for(var i = 0; i <wellData.length; i++){
		if(wellData[i].wellIndex==index){
			...
			layer.open({
				...
				maxmin: true,
				btn:['保存','删除','取消'],
				content: ['/psxj/psh/psh_lr/pshWellInput.html?type=update&SGuid='+SGuid, 'yes'],
...
function addWellShow(){
	layer.open({
		...
		maxmin: true,
		btn:['保存','取消'],
		content: ['/psxj/psh/psh_lr/pshWellInput.html?type=add&SGuid='+SGuid, 'yes'],
		btn1: function(index,layero){
			$(layero).find("iframe")[0].contentWindow.save();
   		}
	});
