...
define([
  ...
  'analyse-tools/lib/hdm-pipe-chart/hdm-pipe-chart',
  'analyse-tools/interfaces/analyse-tool'
],function(on,html,dom,domConstruct,domStyle,domClass,lang,JSON,GraphicsLayer,Graphic,SimpleFillSymbol,SimpleLineSymbol,PictureMarkerSymbol,Color,Polyline,Point,screenUtils,HDMPipeChart,AnalyseTool){
  "use strict";
  return function(){
    var
      map,
      ...
      domMap = {
        miniIconHtml: 
           "<div class=\"hdm-tool-pipe-chart-mini-icon-wrapper\">"
          +"</div>"
      },
      symbols = {
        'SELECT_LINE_SYMBOL': new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID,new Color([255,0,0,0.5]),2),
        'PIPE_LINE_SYMBOL': new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID,new Color([0,0,255]),2)
      },
    ...
    */
   toggle = function(){
     if(domClass.contains(queryMap.main,'hdm-tool-pipe-chart-minimized')){
       maximize();
     }else{
       minimize();
     }
    ...
    */
   maximize = function(){
     html.set(queryMap.miniIcon,'横断面分析图表 -');
     domClass.replace(queryMap.main,'hdm-tool-pipe-chart-maximized','hdm-tool-pipe-chart-minimized');
   ...
   minimize = function(){
     var x, y;
     html.set(queryMap.miniIcon,'横断面分析图表  +');
     domClass.replace(queryMap.main,'hdm-tool-pipe-chart-minimized','hdm-tool-pipe-chart-maximized');
    ...
    */
    initChart = function(options){
      ...
      if(!options.tableContainerId){
        throw "A 'tableContainerId' option is neccessary for init this tool!";
      }
      ...
      chart.init({
        chartParentId: options.chartContainerId,
        tableParentId: options.tableContainerId
      });
    };
    /**
     * 清空横断面分析的图形 
    ...
    /**
     * 进行横断面分析，即利用管线数据在图表上生成管道图像和表格数据
     */
    analyseHDM = function(data){
      var i, pipes, pipeArr = [], graphic;
      pipes = data.features;
      for(i=0;i<pipes.length;i+=1){
        pipeArr.push(pipes[i]);
