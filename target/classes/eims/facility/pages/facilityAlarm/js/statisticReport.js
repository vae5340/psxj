define(['jquery','layer','dateUtil','areaUtil','bootstrap','bootstrapTable','bootstrapTableCN','highcharts','highchartsExporting','highchartsCN'],function($,layer,dateUtil,areaUtil,bootstrap,bootstrapTable,bootstrapTableCN,highcharts,highchartsExporting,highchartsCN){
  var alarmType,estType,deviceOwner,isCheckHis,startTime,endTime;
  ...
  function fillData(data){
  	...
  	$.each(ssTypes,function(index,item){
  		if(item){
  			...
  			$("#statisticReport_columnTable").bootstrapTable("insertRow", { index: item.order-1, row: rowdata });
  		}
  	});
  	if(data){
  		fillPieView(data);
  ...
  function fillPieView(data){
  	$('#statisticReport_pieView').highcharts({
  		chart: {
  ...
  function fillColumnView(data){
  	$('#statisticReport_columnView').highcharts({
  		chart: {
            	type:'column',
            	plotBackgroundColor: null,
            	plotBorderWidth: null,
  ...
  function initColumnTable(){
    $("#statisticReport_columnTable").bootstrapTable({
      toggle:"table",
      rowStyle:"rowStyle",
      cache: false,
      striped: true,    
  ...
  function init( _alarmType,_estType,_deviceOwner,_isCheckHis,_startTime,_endTime) {
    alarmType=_alarmType;
    estType=_estType;
    deviceOwner=_deviceOwner;
    isCheckHis=_isCheckHis;
    startTime=_startTime;
    endTime=_endTime;
    initColumnTable();
    getStatisticData();
    $("#statisticReport_columnTable tr:gt(0)").hover(
        function () { $(this).addClass("hover") },
        function () { $(this).removeClass("hover") }
      )
      $("#statisticReport_pieTable tr:gt(0)").hover(
      function () { $(this).addClass("hover") },
        function () { $(this).removeClass("hover") }
      )
  };
