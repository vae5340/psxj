define(['jquery','layer','dateUtil','areaUtil','bootstrap','bootstrapTable','bootstrapTableCN','highcharts','highchartsExporting','highchartsCN'],function($,layer,dateUtil,areaUtil,bootstrap,bootstrapTable,bootstrapTableCN,highcharts,highchartsExporting,highchartsCN){
  ...
  var alarmType,estType,deviceOwner,errDate;
  ...
  function fillData(data){
    ...
    $.each(ssTypes,function(index,item){
      if(item){
        ...
        $("#statisticReportRun_columnTable").bootstrapTable("insertRow", { index: item.order-1, row: rowdata });
      }
    });
    if(data){
      fillPieView(data);
  ...
  function fillPieView(data){
    $('#statisticReportRun_pieView').highcharts({
      chart: {
  ...
  function fillColumnView(data){
    $('#statisticReportRun_columnView').highcharts({
      chart: {
       type:'column',
       plotBackgroundColor: null,
       plotBorderWidth: null,
  ...
  function initColumnTable(){
    $("#statisticReportRun_columnTable").bootstrapTable({
      toggle:"table",
      rowStyle:"rowStyle",
      cache: false,
      striped: true,    
  ...
  function init(_alarmType,_estType,_deviceOwner,_errDate) {
    ...
    $("#statisticReportRun_columnTable tr:gt(0)").hover(
      function () { $(this).addClass("hover") },
      function () { $(this).removeClass("hover") }
    )
    $("#statisticReportRun_pieTable tr:gt(0)").hover(
      function () { $(this).addClass("hover") },
      function () { $(this).removeClass("hover") }
    )
  };
