define(["jquery"],function($){
	function init(){
		...
		$.ajax({
			...
			success : function(data) {
				districtName=data;	
				for(var index in xzArea){
					if(xzArea[index].name==districtName){
		...
		require([
				...
				featureCollection.layerDefinition = {
					...
					"drawingInfo": {
						"renderer": {	          
							...
							"symbol": {
								"type": "esriPMS",
								"url": "/awater/monitoring/ssjk/img/warning.png",
								"contentType": "image/png",
								"width": iconW,
								"height": iconH
							}					    				
			...
			if(map.getLayer(featureLayreId)){
				...
				switch (featureLayreId){				
					...
					graphicLayerArr.push(featureLayer);			
				}
	...
	function openSPWindow(dimId,itemId,itemType,title,itemTag){
		...
		shade: false,
		area: ['620px', '520px'],
		content: location.protocol+"//"+location.hostname+":"+location.port+'/awater/monitoring/ssjk/ssjk-dim-sx.html' //iframe的url
		}); 
	}
	/**
	 * 打开监控项
	...
	function showJsdDispatch(g){
		$.get("/psemgy/eims/dispatch/pages/district/record/jsdDispatchMapRecord.html",function(h){
		  ...
		  require(['psemgy/eims/dispatch/pages/district/record/js/jsdDispatchMapRecord'],function(jsdDispatchMapRecord){
		    jsdDispatchMapRecord.init(g.graphic.attributes.combId);
		  });
		}); 
		// layer.open({
		// 	type: 2,
		...
		// 	shade: 0,
		// 	area: ['700px', '440px'],
		// 	content:"/psemgy/eims/dispatch/pages/district/record/jsdDispatchMapRecord.html?combId="+g.graphic.attributes.combId+"&modelId="+recordId+"&type=4"
		// }); 
	}

	function showSsjkMain(g){
		if(g.graphic.getLayer().id!="ssjk_17"){
			$.get("/awater/monitoring/ssjk/ssjk-dim-main.html",function(h){ 
		  	layerIndex=layer.open({
			    type: 1, 
			    content: h,
			    title: g.graphic.attributes.title,
			  ...
			  require(['monitoring/ssjk/js/ssjk-dim-main'],function(ssjkDimMain){
			    ssjkDimMain.init(g.graphic.attributes.combId);
			  });
			});  
			// layer.open({
			// 	type: 2,
			...
			// 	shade: 0,
			// 	area: ['860px', '420px'],
			// 	content:  ["/awater/monitoring/ssjk/ssjk-dim-main.html?combId="+g.graphic.attributes.combId,"#mapDiv"] //iframe的url
			// }); 
		}
	}
	...
	function buildClickMenu(g){
		...
		$.ajax({
			url: '/psfacility/pscomb/buildMapClickMenu.action',
	...
	function onClickMenu(combId,title){
		...
		// 	shade: 0,
		// 	area: ['860px', '420px'],
		// 	content: ["/awater/monitoring/ssjk/ssjk-dim-main.html?combId="+combId,"#mapDiv"]
		// });
	}

	return{
		init: init,
		ssjkCreatePoint: ssjkCreatePoint
	}
});	