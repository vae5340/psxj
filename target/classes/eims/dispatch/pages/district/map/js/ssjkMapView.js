...
function addAllMonitorStation(result){
	var specialArea=parent.window.specialArea;
	var area=parent.window.districtArea;
	var rows=result.rows;
	//1 设施
	for(var i=0;i<rows.length;i++){
		var row=rows[i];
		if(!specialArea&&row.area!=area&&row.estType!=17){
		    continue;
	    }
...
function showJsdDispatch(g){
	layer.open({
		  ...
		  shade: 0,
		  area: ['700px', '440px'],
		  content:"/psemgy/eims/dispatch/pages/district/template/jsdDispatch.html?combId="+g.graphic.attributes.combId+"&modelId="+recordId+"&yaType="+2+"&type="+4
		}); 
}
