...
define(['jquery','layer','highcharts','highchartsExporting','highchartsCN','bootstrap','bootstrapTable','bootstrapTableCN','mousewheel','customScrollbar'],function($,layer,highcharts,highchartsExporting,highchartsCN,bootstrap,bootstrapTable,bootstrapTableCN,mousewheel,customScrollbar){
  ...
  function imgFormat(value, row, index){
    if(row.id)
      return "<img src='/awater/pipe/pipeNetwork/img/end-marker.png' height=20, width=14></img>";
    else
      return "<img src='/awater/pipe/pipeNetwork/img/target-marker.png' height=20, width=14></img>";
  };
  ...
  // Create the chart
  /*$('#backflowAnalysis_containerPie').highcharts({
  chart: {
  type: 'pie'
  },
  title: {
  ...
  return this.y > 1 ? '<b>' + this.point.name + ':</b> ' + this.y + '%'  : null;
    ...
    $('#backflowAnalysis_container').highcharts({
      chart: {
        type: 'column'
      },
      title: {
  ...
  function init(){
    $("#backflowAnalysis_resultTable").bootstrapTable({
      data: stations,
      method: 'get',
      detailView: true,
      pageSize: 10,
    ...
    $("#backflowAnalysis_resultTable").on('post-body.bs.table', function (row,obj) {
      $(".fixed-table-body").mCustomScrollbar();        
    });

    $("#backflowAnalysis_resultTable").on('click-row.bs.table', function (row,obj) {
      parent.centerAtCoor(obj.x,obj.y);
    });
